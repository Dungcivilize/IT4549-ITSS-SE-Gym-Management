  ...
  useEffect(() => {
    const fetchFeedbacks = async () => {
      try {
        ...
        const sortedFeedbacks = res.data.sort(
  ...
  const fetchMyFeedbacks = async () => {
    ...
    try {
      const res = await axios.get(
        `http://localhost:8080/api/feedbacks/member/${user_id}`
      );
  ...
  const handleDelete = async (feedbackId) => {
    ...
    try {
      ...
      setError(
        err.response?.data?.message ||
          err.message ||
          'Có lỗi xảy ra khi xóa feedback'
      );
    } finally {
      setLoading(false);
    }
      ...
      <div style={pageStyles.contentWrapper}>
        <div style={pageStyles.profileBox}>
            ...
            >
              ...
              >
                ...
                <button
                  ...
                  onMouseOver={(e) =>
                    (e.target.style.backgroundColor = '#545456')
                  }
                  onMouseOut={(e) =>
                ...
                {myFeedbacks.length === 0 ? (
                  ...
                  myFeedbacks.map((fb, index) => (
                    <div
                      key={fb.feedbackId || `modal-feedback-${index}`}
                      style={pageStyles.feedbackCard}
                    >
          ...
          {showAddModal && (
            ...
            >
              <div
                style={{ ...pageStyles.modalContent, maxWidth: '500px' }}
                onClick={(e) => e.stopPropagation()}
              >
            ...
            {displayedFeedbacks.length === 0 ? (
              ...
              <>
                {displayedFeedbacks.map((fb, index) => (
                  <div
                    key={fb.feedbackId || `main-feedback-${index}`}
                    style={pageStyles.feedbackCard}
                  >
                    <div style={pageStyles.feedbackMeta}>
                      <span>
                    ...
                    <div style={pageStyles.feedbackText}>
                      <strong style={{minWidth: '80px'}}>Nội dung:</strong>
                      {editingId === fb.feedbackId ? (
                        <div>
                          <textarea
                            value={editingText}
                ...
                {!showAllFeedbacks && allFeedbacks.length > 5 && (
                  <div style={{ textAlign: 'center' }}>
                    ...
                    >
                      Hiển thị tất cả feedback ({allFeedbacks.length - 5}{' '}
                      feedback khác)
                    </button>
                  </div>
                )}
